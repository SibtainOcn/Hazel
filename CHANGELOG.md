...
- **Collapsible Shell Console**: Renamed "Logs" to "Shell" and transitioned the console into a professional, collapsible UI component with auto-scroll rotation animations.
- **Responsive Download Layout**: Converted the entire Download screen into a unified scrollable container to prevent UI clipping on smaller devices.
...
- **UI Clipping & Proportions**: Fixed the download completion card getting cut off by the navigation bar and unified font sizes (bodySmall) for media details to ensure a clean information hierarchy.
